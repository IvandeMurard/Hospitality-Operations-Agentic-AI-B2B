"""
Apaleo PMS adapter backed by Apaleo's MCP Server.

Drop-in replacement for ``ApaleoPMSAdapter`` — implements the same
``PMSAdapter`` interface but fetches data via MCP tool calls instead
of raw REST API calls.

Decision HOS-101: MCP > API raw.

Apaleo MCP tool mapping
-----------------------
PMSAdapter method           MCP tool name
--------------------------  -----------------------------------
get_occupancy               apaleo_get_occupancy_metrics
get_revenue                 apaleo_get_revenue_metrics
get_historical_data         apaleo_get_reservations
update_staffing_in_pms      apaleo_update_schedule

Falls back to safe defaults (mock values / False / []) when the MCP
server is unreachable or not configured, so the rest of the pipeline
is never blocked.
"""

from __future__ import annotations

import logging
from datetime import date
from typing import Any

from app.integrations.apaleo_mcp_client import ApaleoMCPClient
from app.services.pms_sync import PMSAdapter

logger = logging.getLogger(__name__)


class ApaleoMCPAdapter(PMSAdapter):
    """
    PMSAdapter implementation using Apaleo's MCP Server.

    Inject a custom ``ApaleoMCPClient`` for testing::

        adapter = ApaleoMCPAdapter(mcp_client=FakeClient())
    """

    def __init__(self, mcp_client: ApaleoMCPClient | None = None) -> None:
        self._client = mcp_client or ApaleoMCPClient()

    # ------------------------------------------------------------------
    # PMSAdapter interface
    # ------------------------------------------------------------------

    async def get_occupancy(self, property_id: str, target_date: date) -> int:
        """Return occupied-rooms count for *property_id* on *target_date*.

        Falls back to 80 on any error so the forecasting pipeline is
        never blocked.
        """
        if not self._client.is_configured:
            logger.warning("ApaleoMCPAdapter: not configured — returning mock occupancy (80)")
            return 80

        try:
            data = await self._client.call_tool(
                "apaleo_get_occupancy_metrics",
                {"propertyId": property_id, "date": target_date.isoformat()},
            )
            # Apaleo may return either "occupancyPercent" (0-100) or raw "occupancy"
            return int(data.get("occupancyPercent", data.get("occupancy", 80)))
        except Exception as exc:
            logger.error("ApaleoMCPAdapter.get_occupancy failed: %s", exc)
            return 80

    async def get_revenue(
        self,
        property_id: str,
        target_date: date,
        category: str = "Total",
    ) -> float:
        """Return revenue for *property_id* on *target_date* filtered by *category*.

        When no items match the requested category, sums all items so the
        caller always receives a non-None float.
        """
        if not self._client.is_configured:
            logger.warning("ApaleoMCPAdapter: not configured — returning 0.0 revenue")
            return 0.0

        try:
            data = await self._client.call_tool(
                "apaleo_get_revenue_metrics",
                {
                    "propertyId": property_id,
                    "from": target_date.isoformat(),
                    "to": target_date.isoformat(),
                    "category": category,
                },
            )
            revenue_items: list[dict[str, Any]] = data.get("revenue", [])
            if not revenue_items:
                return float(data.get("totalRevenue", 0.0))

            matched = [
                float(item.get("amount", 0.0))
                for item in revenue_items
                if category.lower() in item.get("category", "").lower()
            ]
            return sum(matched) if matched else sum(
                float(i.get("amount", 0.0)) for i in revenue_items
            )
        except Exception as exc:
            logger.error("ApaleoMCPAdapter.get_revenue failed: %s", exc)
            return 0.0

    async def get_historical_data(
        self,
        property_id: str,
        start_date: date,
        end_date: date,
    ) -> list[dict[str, Any]]:
        """Return raw reservation records for the requested date range."""
        if not self._client.is_configured:
            logger.warning("ApaleoMCPAdapter: not configured — returning empty history")
            return []

        try:
            data = await self._client.call_tool(
                "apaleo_get_reservations",
                {
                    "propertyId": property_id,
                    "from": start_date.isoformat(),
                    "to": end_date.isoformat(),
                    "pageSize": 500,
                },
            )
            # Apaleo wraps items under "reservations"; handle bare-list fallback
            if isinstance(data, list):
                return data
            return data.get("reservations", [])
        except Exception as exc:
            logger.error("ApaleoMCPAdapter.get_historical_data failed: %s", exc)
            return []

    async def update_staffing_in_pms(
        self,
        property_id: str,
        target_date: date,
        staffing_deltas: dict[str, int],
    ) -> bool:
        """Push staffing deltas to Apaleo's schedule via the MCP tool.

        Returns ``True`` only when the MCP server confirms success.
        """
        if not self._client.is_configured:
            logger.warning("ApaleoMCPAdapter: not configured — staffing push skipped")
            return False

        try:
            data = await self._client.call_tool(
                "apaleo_update_schedule",
                {
                    "propertyId": property_id,
                    "date": target_date.isoformat(),
                    "deltas": staffing_deltas,
                    "source": "Aetherix-AI",
                },
            )
            # Tolerate different success indicators from Apaleo's response
            success = bool(
                data.get("success")
                or data.get("status") in ("ok", "created", "updated", "accepted")
            )
            logger.info(
                "ApaleoMCPAdapter.update_staffing: property=%s date=%s success=%s",
                property_id,
                target_date,
                success,
            )
            return success
        except Exception as exc:
            logger.error("ApaleoMCPAdapter.update_staffing_in_pms failed: %s", exc)
            return False
