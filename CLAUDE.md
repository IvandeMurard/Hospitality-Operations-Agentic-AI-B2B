...
- [ ] **Apaleo MCP Server URL** : `APALEO_MCP_SERVER_URL` actuellement hypothétique (`https://mcp.apaleo.com`). Confirmer l'URL réelle dès publication par Apaleo. Sources de veille → HOS-101 (commentaire posté 29/03/2026). Critères de mise à jour : `.env.example` + `CLAUDE.md` + test live `POST /pms/sync?use_mcp=true`.
