...
- [ ] **Apaleo MCP — Composio connecté, scope limité (30/03/2026)** : `ApaleoMCPClient` supporte deux modes — `apikey` (Composio, dispo) et `bearer` (Apaleo direct alpha, invite requise). ⚠️ Composio expose uniquement l'**Inventory API** (properties/units) — PAS occupancy/revenue/reservations. Pour les données F&B, `ApaleoPMSAdapter` (raw REST) reste le path principal. `ApaleoInventoryMCPService` = nouveau service pour setup propriété via Composio. Script de génération URL : `scripts/generate_composio_mcp_url.py`. Env vars : `COMPOSIO_API_KEY`, `COMPOSIO_USER_ID`, `COMPOSIO_MCP_URL`. Accès alpha direct Apaleo = next step pour couvrir F&B via MCP.
