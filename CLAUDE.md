...
- [ ] **Apaleo MCP Server URL** : deux chemins identifiés (30/03/2026) — (1) **Composio managed** : URL dynamique + `X-API-Key` auth, dispo maintenant via `mcp.composio.dev/apaleo` ; (2) **Apaleo direct** : alpha fermé, URL non publiée, accès sur invitation via Agent Hub. `ApaleoMCPClient` supporte déjà les deux (OAuth2 Bearer pour direct, header custom pour Composio). Prochaine étape : accès alpha Apaleo OU compte Composio. Voir HOS-101 commentaires.
